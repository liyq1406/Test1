...
+ (NSMutableAttributedString *)setHHGGTextWithDatas:(FBDatasModel *)model;
{
    ...
    if (model.scroes)return [self setScroeTextWithDatas:model.scroes];
    if (model.BQCDatas) return [self setBQCTextWithDatas:model.BQCDatas];
...
#pragma mark - 过关投注结果
+ (void)calculateSkipmatchDatas:(NSArray *)datas scheme:(NSMutableArray *)scheme result:(void(^)(NSInteger bettingNum))result;
{
