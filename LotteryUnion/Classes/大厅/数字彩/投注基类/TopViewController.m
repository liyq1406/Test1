...
-(BOOL)betResponse:(id)response lot_pk:(int)lot_pk buttonTag:(NSInteger)btnTag
{
    ...
    { // 成功
        ...
        // 显示投注成功的alertView
        [self showSuccessWithStatus:betNote];
                 ...
                 Lotter_pk:(int)lot_pk
{
        ...
        switch (buttonIndex)  //查看
          {
            //不管点击了哪个，都要清空中奖信息
             ...
             case 1:   //分享
                  {
                      [ShareTools shareAllButtonClickHandler:shareNote.length?shareNote:@"中奖咯！小伙伴们，快来和我一起加入体彩投注吧~" andUser:nil andUrl:APPDownloadAdrees andDes:@"快来加入彩票联盟吧~"];
                      [UserInfo sharedInstance].Bonus = @"";
                      [UserInfo sharedInstance].ShareBonus = @"";
                      [alertView close];
 ...
 {
       //背景
       UIImageView *view = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, mScreenWidth-80, 260)];
        view.image = [UIImage imageNamed:@"tishi_bg"];
        view.layer.cornerRadius = 8;// 设置圆角
 
 
       //提示
           UIImageView *topLabelImg = [[UIImageView alloc] initWithFrame: CGRectMake(-27.5, 10, view.width+55, 38)];
     ...
     label.text = [NSString stringWithFormat:@"%@中了100.00元",getLotNames(GET_INT((long)lot_pk))];
