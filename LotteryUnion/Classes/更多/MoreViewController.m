...
- (void)checkVersionUpdate
{
    ...
    _connection = [RequestModel POST:URL(kAPI_QueryVersion) parameter:params   class:[RequestModel class]
                   ...
                   {
                       // 升级提示
                       NSString *cancel = [[data objectForKey:@"force_update"] isEqualToString:@"0"]? nil : LocStr(@"取消");
                       NSString *releaseNote = [data objectForKey:@"release_note"];
                       
                       // 判断版本是否有更新
                       if ([[data objectForKey:@"ver"] intValue]>[getBuildVersion() intValue]){
...
-(void)shareToFriend
{
    [ShareTools shareAllButtonClickHandler:@"彩票联盟客户端" andUser:@"最新彩票O2O强势推出，欢迎下载体验。" andUrl:APPDownloadAdrees andDes:@"快来下载吧~"];
}
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex; {
    if (alertView.tag==10087&&buttonIndex == 1) {
        ...
        [[UIApplication sharedApplication] openURL:[NSURL URLWithString:APPDownloadAdrees]];
