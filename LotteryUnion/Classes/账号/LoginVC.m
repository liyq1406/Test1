...
-(void)creatLoginUI
{
    ...
    sendMsgBtn.enabled = [XHDHelper isMobileNumber:phoneTextField.text];
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    if (textField == phoneTextField) {
        ...
        if (detailString.length == 11)//手机号码等11位时，改变按钮颜色
        {
            
            [sendMsgBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
            
//   [self changeMesBtnWithString:(NSMutableAttributedString*)@"获取验证码" WithEnable:YES];
            sendMsgBtn.enabled = YES;
        }
...
- (void)loginButtonClick
{
    ...
    if ([phoneTextField.text length]!=11 ||![phoneTextField.text hasPrefix:@"1"]) {
        ...
        return;
    } else if(!isVCodeLogin&&(pwdLen<6||pwdLen>16)){
